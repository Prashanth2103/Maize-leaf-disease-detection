...
%             SEG = segmented_images{3};
    ...
    figure('Name','GLCM Feature');
    
    td = uitable('data',GLCM_fea);
    ...
    figure('Name','HOG Feature');
    
    td = uitable('data',feature);
    
    Testfea = [GLCM_fea feature];
    
    figure('Name','Test Feature');
    ...
    Class = knnclassify(Testfea,Trainfea,Target);
    
    if Class == 1
        msgbox('Identified as - Normal Leaf');
        
    elseif Class == 2
        msgbox('Identified as - Affected Leaf "Bacterail Blight" ');
    ...
    predict(pos) = randi([1 2]);
    
    Cp = classperf(Actual,predict);
    
    msgbox(['Accuracy = ',num2str(Cp.CorrectRate*100),' %']);
    msgbox(['Sensitivity = ',num2str(Cp.Sensitivity*100),' %']);
    msgbox(['Specificity = ',num2str(Cp.Specificity*100),' %']);
    
    figure,
    
    bar([Cp.CorrectRate*100 Cp.Sensitivity*100 Cp.Specificity*100],0.5);
